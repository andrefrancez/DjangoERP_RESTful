...
from companies.serializers import TaskSerializers, SpecificTaskSerializers
...
class Tasks(Base):
    ...
    def post(self, request):
        ...
        if not title or len(title) > 125:
            raise APIException('Send a valid title for this task')

        if due_date:
            try:
                due_date = datetime.datetime.strptime(due_date, '%d/%m/%Y %H:%M')
            except ValueError:
                raise APIException('The date must follow the standard: d/m/Y H:M', 'date_invalid')
        ...
        serializer = SpecificTaskSerializers(task)
...
class TaskDetail(Base):
    ...
    def put(self, request, task_id):
        ...
        serializer = SpecificTaskSerializers(task, data=data, partial=True)

        if not serializer.is_valid():
            raise APIException('It was not possible to edit the task')
